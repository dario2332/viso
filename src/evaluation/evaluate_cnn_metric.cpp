...
float CNNCost(const vector<float> &d1, const vector<float> &d2) {
  return cblas_sdot(64, &d1[0], 1, &d2[0], 1);
}
...
int main (int argc, char** argv) {
  ...
  ConvMatcher *matcher  = new ConvMatcher(param.match, graph_path);
  ...
  for (int32_t i=0; i<num_frames; i++) {
    ...
    try {
      ...
      if (i == 0) matcher->setDims(width, height);
      matcher->pushBackFetures(img_file_name, img_file_name);

      for (int v = 0; v < 16; v++) {
        for (int u = 0; u < 16; u++) {
          vector<float> d(64);
          matcher->computeDescriptor(u*width/16 + width/16/2, v*height/16 + height/16/2, (float*) &d[0]);
  ...
  for (int i = 0; i < descriptors.size(); ++i) {
    for (int j = i+1; j < descriptors.size(); ++j) {
      ...
      float result = CNNCost(descriptors[i], descriptors[j]);
  ...
  while (pos_examples > 0) {
    ...
    if (true_labels[first_elem] != true_labels[second_elem]) {
      ...
      float result = CNNCost(descriptors[first_elem], descriptors[second_elem]);
      scores.push_back(result);
      pos_examples--;
    }
  }
