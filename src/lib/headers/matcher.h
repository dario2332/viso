...
public:
  ...
  struct parameters {
    ...
    int32_t sort;                   // 0=no sorting, 1=sort matches by age
    bool use_initial_descriptor;
...
private:
  ...
  // sort matches by age
  void sortMatches(std::vector<p_match> &matches);

  //sorting function
  bool compareWithMaxima(const p_match &match1, const p_match &match2);

  //update matches age
  void updateMatches(std::vector<p_match> &matches);
