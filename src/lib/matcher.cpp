...
bool Matcher::compareWithMaxima(const p_match &match1, const p_match &match2) {
  int size = sizeof(Matcher::p_match) / sizeof(int32_t);
  int index1 = match1.i1p;
  int index2 = match2.i1p;
  int add1 = size*index1;
  int add2 = size*index2;
  int val1 = m1p2[add1+2];
  int val2 = m1p2[add2+2];
  return val1 > val2;
}

void Matcher::updateMatches(vector<p_match> &matches) {
  int size = sizeof(Matcher::p_match) / sizeof(int32_t);
  for (int i = 0; i < matches.size(); ++i) {
    int add_p1 = size*matches[i].i1p;
    int add_c1 = size*matches[i].i1c;
    int add_p2 = size*matches[i].i2p;
    int add_c2 = size*matches[i].i2c;
    m1c2[add_c1+2] = m1p2[add_p1+2]+1;
    
    if (param.use_initial_descriptor) {
      for (int j = 4; j < 12; ++j) {
        m1c2[add_c1+j] = m1p2[add_p1+j];
        m2c2[add_c2+j] = m2p2[add_p2+j];
      }
    }
  }
}
